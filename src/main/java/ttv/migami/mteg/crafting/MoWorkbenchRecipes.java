...
public class MoWorkbenchRecipes
{
    ...
    {
        return world.getRecipeManager().getRecipes().stream().noneMatch(recipe -> recipe.getType() == ModRecipeTypes.MO_WORKBENCH);
    }

    public static NonNullList<MoWorkbenchRecipe> getAll(Level world)
    {
        return world.getRecipeManager().getRecipes().stream()
                .filter(recipe -> recipe.getType() == ModRecipeTypes.MO_WORKBENCH)
