...
public class ScarLModel implements IOverrideModel {
	...
	public void render(float partialTicks, TransformType transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
		...
		if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.WEIGHTED_STOCK.get())
			RenderUtil.renderModel(SpecialModels.SCAR_L_STOCK_HEAVY.getModel(), stack, matrixStack, buffer, light, overlay);
		...
		if(entity.equals(Minecraft.getInstance().player)) {
            ...
            RenderUtil.renderModel(SpecialModels.SCAR_L_EJECTOR.getModel(), stack, matrixStack, buffer, light, overlay);
            //Always pop
            matrixStack.popPose();
