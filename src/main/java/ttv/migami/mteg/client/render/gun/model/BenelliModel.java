...
public class BenelliModel implements IOverrideModel {
	...
	public void render(float partialTicks, ItemDisplayContext ItemDisplayContext, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
		...
		RenderUtil.renderModel(SpecialModels.BENELLI_M4_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);

        if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.LIGHT_STOCK.get())
            RenderUtil.renderModel(SpecialModels.BENELLI_M4_STOCK_LIGHT.getModel(), stack, matrixStack, buffer, light, overlay);
        
        if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.TACTICAL_STOCK.get())
            RenderUtil.renderModel(SpecialModels.BENELLI_M4_STOCK_TACTICAL.getModel(), stack, matrixStack, buffer, light, overlay);
        
        if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.WEIGHTED_STOCK.get())
            RenderUtil.renderModel(SpecialModels.BENELLI_M4_STOCK_HEAVY.getModel(), stack, matrixStack, buffer, light, overlay);
		...
		if(entity.equals(Minecraft.getInstance().player)) {
            ...
            RenderUtil.renderModel(SpecialModels.BENELLI_M4_PUMP.getModel(), stack, matrixStack, buffer, light, overlay);
            //Always pop
            matrixStack.popPose();
